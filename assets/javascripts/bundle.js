(() => {
  ...
  function renderAnnotation(id, content) {
    ...
    }, Array.from(content.childNodes))), /* @__PURE__ */ h("span", {
  ...
  // src/assets/javascripts/components/content/code/_/index.ts
  var index = 0;
  function watchCodeBlock(el, { viewport$: viewport$3 }) {
  ...
  function mountDetails(el, options) {
    ...
    return watchDetails(el, options).pipe(tap((state) => internal$.next(state)), finalize(() => internal$.complete()), mapTo({ ref: el }));
  }
  ...
  function watchContentTabs(el) {
    return of(getElements(":scope > label", el)).pipe(switchMap((labels) => of(...labels).pipe(mergeMap((label) => {
  ...
  function setupSearchIndex({ config: config4, docs, index: index3 }) {
    ...
    return { config: config4, docs, index: index3, options };
  }
